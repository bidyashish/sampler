          ...
          <div class="w-2/3 mx-auto">
             <div class="bg-white rounded my-6">
                <table class="text-left w-full border-collapse userlist">
                   ...
                   <tr class="modal-open" data-receiverid="{{$user->id }}" data-senderid="{{Auth::user()->id }}"  data-receiverfname="{{$user->fname }}" data-receivermaxamount="{{Auth::user()->balance }}" onclick="activateModal(this);"  >
          ...
          <div class="modal-container bg-gray w-11/12 md:max-w-sm mx-auto  shadow-lg  z-50 overflow-y-auto">
             <div class="bg-white p-4 ">
                ...
                <form method="POST" action="{{ route('dashboard') }}">
                   ...
                   <h3 class="font-bold text-center">Send Money to <span id="receiverfname"></span></h3>
                   <!-- Amount -->
                   <div>
                      <x-label for="amount" :value="__('Amount')" />
                      $
                      <x-input id="amount" class="block mt-1 w-full" step=".01" placeholder="0.00" type="number" name="amount" :value="old('amount')" required autofocus />
                   </div>
                   <div class="  items-center  mt-4">
                      <button class="modal-close px-4 bg-transparentp-3  border-gray-500 text-grey hover:bg-green-500">Close</button>
                      <button type="submit" class="  items-center px-4 py-2 bg-blue-400 border border-transparent  font-semibold text-md text-white  tracking-widest hover:bg-green-500 active:bg-green-500 focus:outline-none focus:border-green-500 focus:ring ring-green-500 disabled:opacity-25 transition ease-in-out duration-150">
                      Send
                      </button>
                   </div>
                </form>
